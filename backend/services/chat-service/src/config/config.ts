...
const envSchema = Joi.object({
  ...
  REQUEST_SERVICE_URL: Joi.string().uri().required(),
  REQUEST_SERVICE_TIMEOUT_MS: Joi.number().integer().min(100).default(5000),
  OFFER_SERVICE_URL: Joi.string().uri().required(),
  OFFER_SERVICE_TIMEOUT_MS: Joi.number().integer().min(100).default(5000),
...
const config = {
  ...
  requestService: {
    url: value.REQUEST_SERVICE_URL as string,
    timeoutMs: Number(value.REQUEST_SERVICE_TIMEOUT_MS)
  },
  offerService: {
    url: value.OFFER_SERVICE_URL as string,
    timeoutMs: Number(value.OFFER_SERVICE_TIMEOUT_MS)
  },
  pagination: {
