...
export const handleChangePasswordRequest = async (
  ...
  const payload = req.payload as { userID?: unknown; userId?: unknown; sub?: unknown } | undefined;
  ...
  try {
    ...
    const emailSent = await sendPasswordChangeEmail(user.email, pendingToken);

    if (!emailSent) {
      return res.status(config.nodeEnv === 'production' ? httpStatus.SERVICE_UNAVAILABLE : httpStatus.OK).json({
        message: 'Password change confirmation email could not be sent.',
        ...(config.nodeEnv !== 'production' ? { confirmationToken: pendingToken } : {})
      });
    }
