...
import { OfferModuleAdapterLike } from './offer-module.adapter';
...
export class ChatService {
  constructor(
    ...
    private readonly offerModuleAdapter: OfferModuleAdapterLike
  ) {}
  ...
  private async resolveSellerId(input: CreateConversationInput): Promise<string> {
    ...
    if (input.offerId !== undefined) {
      const offer = await this.offerModuleAdapter.getOfferById(input.offerId);
      if (offer != null) {
  ...
  private async validateOfferContext(offerId: string, requestId: string, sellerId: string): Promise<void> {
    const offer = await this.offerModuleAdapter.getOfferById(offerId);
