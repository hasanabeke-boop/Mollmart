...
const envSchema = Joi.object({
  ...
  .custom((value, helpers) => {
    ...
    const smtpSet = smtpParts.filter((part) => part.length > 0).length;
    if (smtpSet !== 0 && smtpSet !== smtpParts.length) {
      return helpers.error('any.custom', {
        message: 'SMTP: set SMTP_HOST, SMTP_USERNAME, and SMTP_PASSWORD, or leave all empty'
      });
    }
    if (value.NODE_ENV === 'production' && smtpSet !== smtpParts.length) {
      return helpers.error('any.custom', {
        message: 'Production email delivery requires SMTP_HOST, SMTP_USERNAME, and SMTP_PASSWORD'
      });
    }
    if (value.NODE_ENV === 'production' && value.EMAIL_FROM === 'no-reply@example.com') {
      return helpers.error('any.custom', {
        message: 'Production EMAIL_FROM must be your configured SMTP sender address'
      });
    }
    return value;
  })
...
const smtpEmailEnabled =
  String(value.SMTP_HOST).trim().length > 0 &&
  String(value.SMTP_USERNAME).trim().length > 0 &&
  String(value.SMTP_PASSWORD).trim().length > 0;
