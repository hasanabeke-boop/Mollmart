...
export default function CheckoutPage() {
  ...
  const [form, setForm] = useState(EMPTY_DEMO_CHECKOUT);
  ...
  const submit = async (event: FormEvent) => {
    ...
    const validationError = validateDemoCheckout(form);
    if (validationError) {
  ...
  const payReady = validateDemoCheckout(form) == null;
    ...
    <main className="app-page app-page-narrow">
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-[#4c9a66]">
          Demo checkout simulates payment, creates orders, and reserves stock. All prices are in KZT (₸).
        </p>
      </div>
      ...
      <form onSubmit={submit} className="grid gap-6 md:grid-cols-[1fr_280px] lg:grid-cols-[1fr_320px]">
        <div className="space-y-6">
          <section className="rounded-xl border border-[#cfe7d7] bg-white p-5">
            <h2 className="text-lg font-black text-[#0d1b12]">Shipping & payment</h2>
            <p className="mt-1 text-sm text-[#4c9a66]">
              This is a demo payment for the order. No real card is charged.
            </p>
            <div className="mt-4">
              <DemoPaymentFields
                card={form}
                onCardChange={(patch) => setForm((s) => ({ ...s, ...patch }))}
                shipping={form}
                onShippingChange={(patch) => setForm((s) => ({ ...s, ...patch }))}
              />
            </div>
          </section>
        </div>
        ...
        <aside className="h-fit rounded-xl border border-[#cfe7d7] bg-white p-5">
          ...
          <div className="mt-5 space-y-3 border-t border-slate-200 pt-4">
            ...
            <p className="text-xs text-[#4c9a66]">All totals are in KZT (₸). Shipping is 0 in demo mode.</p>
          </div>
          <button
            type="submit"
